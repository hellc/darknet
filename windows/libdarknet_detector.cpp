...
YOLODLL_API void Detector::Load(std::string cfg_filename, std::string weight_filename, int gpu_id)
{
	int old_gpu_index;
#ifdef GPU
	cudaGetDevice(&old_gpu_index);
...
YOLODLL_API std::vector<bbox_t> Detector::detect(std::string image_filename, float thresh, bool use_mean)
{
	...
	*image_ptr = load_image(image_filename);
	std::vector<bbox_t> results = detect(*image_ptr, thresh, use_mean);
	return results;
}
